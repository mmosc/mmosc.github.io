...
- **Everything else Python.** [`requirements.txt`](requirements.txt) is the fuller list and must be installed separately (`python3 -m pip install -r requirements.txt`): `rendercv[full]` for CV rendering, `scholarly` for `bin/update_scholar_citations.py`, `bibtexparser` for `bin/sync_bib.py`, plus `nbconvert` and `pyyaml`.
- **Bibliography sync.** `bin/sync_bib.py` keeps `_pages/publications.md`'s `years:` front matter, per-entry `preview` images (matched against `assets/img/publication_preview/`), and a required custom-field schema (`bibtex_show`, `selected`, `month`, `topic`, `venue_short`, `first_author`, `phd_relevant`) in sync with `_bibliography/papers.bib`. Manually invoked, no CI: run `python3 bin/sync_bib.py` after editing `papers.bib`. It computes every intended edit and every unresolved issue up front — if anything is ambiguous or a required field is missing, it prints a report and writes nothing (exit 1); only a fully-resolved run writes files (exit 0). See `tasks/plan.md` on the `bib-sync` branch for the full design rationale, or [`ADDING_A_PAPER.md`](ADDING_A_PAPER.md) for the plain-language, non-agent walkthrough of the whole new-paper workflow.
