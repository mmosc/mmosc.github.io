...
On-card venue text is shortened to an organization + conference acronym (e.g. "Proceedings of the 18th ACM Conference on Recommender Systems (RecSys)" → "ACM RecSys"), stripping "Proceedings of"/"Proc. of" wrappers; the full official name is still available on hover. Phase 3 needs a decision on where this shortened form comes from once cards are driven by real `_bibliography/papers.bib` data instead of a hand-copied preview array — likely a new curated `venue_short` bib field, consistent with how `topic`/`first_author`/`month` were each added deliberately rather than auto-derived (see `tasks/plan.md`'s venue-shortening addendum).
...
Note: the bar's time→pixel scale is fixed and never rescaled, so segment lengths are exactly proportional to elapsed time (verified: 736.4 px/year on every real job segment, zero deviation). The bar and its container are separately sized to always fully contain whatever room the packed paper cards need (some real papers share an exact publication month, which forces a fixed minimum card-to-card gap no amount of rescaling can remove) — any reserved space past the true end of the timeline is left blank rather than stretching the time axis. See `tasks/plan.md`'s "bar length not matching required content height" addendum for the full fix and the diverging approach it replaced.
